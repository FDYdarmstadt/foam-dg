...
void BoSSS_SIPLaplacianScheme<Type, GType>::calcEta()
{
    Info << "Hello from BoSSS_SIP" << endl;
...
BoSSS_SIPLaplacianScheme<Type, GType>::dgcLaplacian
(
    const dimensionedScalar& gamma,
    const DgGeometricField<Type, dgPatchField, cellMesh>& vf
)
{
    Info << "Helo from BoSSS_SIP (2)" << endl;
...
template<class Type, class GType>
tmp<dgMatrix<Type> >
BoSSS_SIPLaplacianScheme<Type, GType>::dgmLaplacian
(
    const dimensionedScalar& gamma,
    const DgGeometricField<Type, dgPatchField, cellMesh>& vf
)
{
    Info << "Helo from BoSSS_SIP (3)" << endl;
