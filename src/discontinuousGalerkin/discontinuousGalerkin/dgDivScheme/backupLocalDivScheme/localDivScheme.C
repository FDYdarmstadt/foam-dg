...
//    scalarField volumeCoeffs(ds[0].size(), 0.0);
...
//Info << "CELLS SCALE: " << scaleCells[cellI] << endl;
//Info << "VELOCITY INT: " << velocityInt << endl;
                scalarInt[cellI] +=
                vsf[cellI][modI]*polyEvalW[ptI][modI]/2;//*scaleCells[cellI];

                Info << pti
                     << " VSF VAL: " << vsf[cellI]
                     << "POLYEVAL: " << polyEvalW[ptI][modI]
                     << " scale: " << scaleCells[cellI]
                     << " pos value: " << Foam::sign(- scalarInt[cellI])
                     << " READL VALUE: " << scalarInt[cellI]
                     << endl;
            }
...
//    }
    ...
    Info << "SCALAR CELL VALUE: " << sign(-scalarInt) << endl;
...
//
                    ...
                    volumeCoeffs[coeff] +=
                    velocityInt[cellI]*polyEvalW[ptI][modI]*polyGEval[ptI][modJ];

                    ds[cellI](modJ, modI) += -
                    velocityInt[cellI]*polyEval[ptI][modI]*polyGEval[ptI][modJ];
//                    *scaleCells[cellI];


//Info << "VOLUME CEOFFS: " << volumeCoeffs[coeff] << ", last term: "
//     << velocityInt << ", point: " << pti << ", weighter poly: " << polyEvalW[ptI][modI] <<
//     ", weightedGrad " <<
//     polyGEval[ptI][modJ] << ", mod index: " << modJ << ", " << modI
//     << endl;
                }
...
//Info << "CELL DIAG: " << cellI << ", is: " << ds[cellI] << endl;
...

{
    ...
    forAll (neiList, faceI)
    {
        ...
        vector gaussPtVec (gaussPt, 0, 0);
        vector gaussPtVecNei (gaussPtNei, 0, 0);
...
//Info << "FOR FACE " << faceI << ", velocity: " << faceUOwn << ", and flux own: "
//     << fluxDirOwn << ", flux nei: " << fluxDirNei << endl;
        ...
        scalarList scalarIndicator = mag(sign(-scalarInt) - 1)/2.0;


//        forAll (scalarIndicator, cellI)
//        {
//            scalarIndicator[cellI] = sign(-scalarInt[cellI]);
//        }

        Info << " SCALAR INDICATORE: " << scalarIndicator <<
             " and " << scalarInt << endl;
...
//            Info << "VELOCITY OWN: " << velocityOwn << endl;
//            Info << "VELOCITY NEI: " << velocityNei << endl;
        }
        ...
        forAll(A, modJ)
        {
            forAll(A, modI)
            {
                ...
                Aown[coeff] =
                velocityOwn*polyEval[modJ]*polyEval[modI]*scalarIndicator[own];

                Anei[coeff] = velocityNei*polyEvalNei[modJ]*polyEvalNei[modI];

                AownNei[coeff] = 0.5*velocityOwn*polyEvalNei[modJ]*polyEval[modI];

                AneiOwn[coeff] = 0.5*velocityNei*polyEval[modJ]*polyEvalNei[modI];
...
//                        vvf[own][0][inner]
//                         + vvf[nei][0][inner])
//                        *surfaceCoeffsOwn[coeff][inner]);
////                        mag(vvf[own][0][inner]*surfaceCoeffsOwn[coeff][inner]);
//
//                    Anei[coeff] +=
//                        mag(
//                        0.5*(
//                        vvf[nei][0][inner]
//                         + vvf[own][0][inner])
//                        *surfaceCoeffsNei[coeff][inner]);
////                        mag(vvf[nei][0][inner]*surfaceCoeffsNei[coeff][inner]);
//
////                    AownNei[coeff] +=
...
Info << " CELL: " << own << ", vec: " << velocityOwn
     << " and cell: " <<  nei << ", vec: " << velocityNei
     << ", surface Coeff own: " << Aown
     << " and surfaceCoeff nei: :" << Anei << endl;
...
//                if (fluxDirOwn == 0.0)
                if (faceFlux == 0.0)
                {}
//                else if (fluxDirOwn > 0.0)
                else if (faceFlux > 0.0)
                {
                ls[faceI](coeffJ, coeffI) += Aown[mtxCoeff];
//                us[faceI](coeffJ, coeffI) += -Aown[mtxCoeff]+Anei[mtxCoeff];
//                      ds[own](coeffJ, coeffI) += Aown[mtxCoeff];


//                      ls[faceI](coeffJ, coeffI) += AownNei[mtxCoeff];
//                      us[faceI](coeffJ, coeffI) += AneiOwn[mtxCoeff];
//                      ds[own](coeffJ, coeffI) += Aown[mtxCoeff];
//                      ds[nei](coeffJ, coeffI) += Anei[mtxCoeff];
                ...
                {
//                ls[faceI](coeffJ, coeffI) += - Aown[mtxCoeff];
//                us[faceI](coeffJ, coeffI) += - Anei[mtxCoeff];
//                      ds[nei](coeffJ, coeffI) += - Anei[mtxCoeff];
...
//                if (fluxDirNei == 0)
//                {}
//                else if (fluxDirNei < 0.0)
//                {
//                      us[faceI](coeffJ, coeffI) +=  0.5*diagCoeffNei[mtxCoeff];
//                }
//                else
//                {
//                      ls[faceI](coeffJ, coeffI) +=  0.5*diagCoeffOwn[mtxCoeff];
//                }
            }
        }
