...
//    dgm.diag() = rDeltaT*mesh.mesh().cellVolumes();///mesh.mesh().cellVolumes();
    ...
    forAll (vf, cellI)
    {
        forAll (A, coeffI)
        {
            forAll (A, coeffJ)
            {
//                ds[cellI](coeffI, coeffJ) =// rDeltaT;
//                rDeltaT*mesh.mesh().cellVolumes()[cellI];
                dgm.diag() =
                rDeltaT*mesh.mesh().cellVolumes();///mesh.mesh().cellVolumes();
            }
        }
    }

//    ds[0] = rDeltaT*mesh.mesh().cellVolumes();///mesh.mesh().cellVolumes();
...
Info << "DDT SOURCE old time: " << dgm.source() << endl;
    ...
    dgm.source() = //rDel;
    rDeltaT*vf.oldTime().internalField()*mesh.mesh().cellVolumes();
//    rDeltaT*vf.oldTime().internalField();
